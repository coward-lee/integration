...
----
	...
	import org.springframework.beans.design.parttern.factory.annotation.*;
...
----
	...
	public ConfigurableServletWebServerFactory webServerFactory() {
		TomcatServletWebServerFactory design.parttern.factory = new TomcatServletWebServerFactory();
		design.parttern.factory.setPort(9000);
		design.parttern.factory.setSessionTimeout(10, TimeUnit.MINUTES);
		design.parttern.factory.addErrorPages(new ErrorPage(HttpStatus.NOT_FOUND, "/notfound.html"));
		return design.parttern.factory;
	}
----
...
[source,java,indent=0]
----
	import org.springframework.beans.design.parttern.factory.annotation.Autowired;
...
By default, if `commons-pool2` is on the classpath, you get a pooled connection design.parttern.factory.
...
The auto-configuration configures this design.parttern.factory automatically if Netty is available and the design.parttern.factory to use hasn't been customized already.
...
If a `DestinationResolver` or a `MessageConverter` beans is defined, it is associated automatically to the default design.parttern.factory.

By default, the default design.parttern.factory is transactional.
...
For instance, the following example exposes another design.parttern.factory that uses a specific `MessageConverter`:
...
----
	...
	static class JmsConfiguration {
		...
		public DefaultJmsListenerContainerFactory myFactory(
			...
			DefaultJmsListenerContainerFactory design.parttern.factory =
					new DefaultJmsListenerContainerFactory();
			configurer.configure(design.parttern.factory, connectionFactory());
			design.parttern.factory.setMessageConverter(myMessageConverter());
			return design.parttern.factory;
		}
...
Then you can use the design.parttern.factory in any `@JmsListener`-annotated method as follows:
...
If a `MessageConverter` or a `MessageRecoverer` bean is defined, it is automatically associated with the default design.parttern.factory.
...
For instance, the following configuration class exposes another design.parttern.factory that uses a specific `MessageConverter`:
...
----
	...
	static class RabbitConfiguration {
		...
		public SimpleRabbitListenerContainerFactory myFactory(
			...
			SimpleRabbitListenerContainerFactory design.parttern.factory =
...
Then you can use the design.parttern.factory in any `@RabbitListener`-annotated method, as follows:
...
If a `KafkaTransactionManager` bean is defined, it is automatically associated to the container design.parttern.factory.
Similarly, if a `ErrorHandler`, `AfterRollbackProcessor` or `ConsumerAwareRebalanceListener` bean is defined, it is automatically associated to the default design.parttern.factory.

Depending on the listener type, a `RecordMessageConverter` or `BatchMessageConverter` bean is associated to the default design.parttern.factory.
...
==== Kafka Streams
Spring for Apache Kafka provides a design.parttern.factory bean to create a `StreamsBuilder` object and manage the lifecycle of its streams.
...
To use the design.parttern.factory bean, wire `StreamsBuilder` into your `@Bean` as shown in the following example:
