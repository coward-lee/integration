...
public class MultiClientTwo {

    int port;
    String ip;

    public MultiClientTwo(String ip, int port) {
        this.port = port;
        this.ip = ip;
    }
...
//                ch.pipeline().addLast(new Decoder());
        ...
        for (int i = 0; i < 10; i++) {
            Thread.sleep(1000);
            ByteBuf buf = Unpooled.copiedBuffer("bbbbb".getBytes());
    ...
    static class Decoder extends MessageToMessageDecoder<String> {
        ...
        protected void decode(ChannelHandlerContext ctx, String msg, List<Object> out) throws Exception {
            System.out.println("ClientOutChannel BBBBBBBBB 客户端 收到了：【"+msg+"】");
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        new MultiClientTwo("127.0.0.1",80).runClient();
    }
}