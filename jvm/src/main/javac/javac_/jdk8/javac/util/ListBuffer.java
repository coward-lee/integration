...
public class ListBuffer<A> extends AbstractQueue<A> {
    ...
    public final void clear() {
        this.elems = com.sun.tools.javac.util.List.nil();
        this.last = null;
        count = 0;
        shared = false;
    }
    ...
    private void copy() {
        if (elems.nonEmpty()) {
            com.sun.tools.javac.util.List<A> orig = elems;

            elems = last = com.sun.tools.javac.util.List.<A>of(orig.head);

            while ((orig = orig.tail).nonEmpty()) {
                last.tail = com.sun.tools.javac.util.List.<A>of(orig.head);
                last = last.tail;
            }
        }
    }
    ...
    public ListBuffer<A> append(A x) {
        ...
        if (shared) copy();
        com.sun.tools.javac.util.List<A> newLast = com.sun.tools.javac.util.List.<A>of(x);
        if (last != null) {
            last.tail = newLast;
            last = newLast;
        } else {
     ...
     */
    public ListBuffer<A> appendList(com.sun.tools.javac.util.List<A> xs) {
        while (xs.nonEmpty()) {
            append(xs.head);
            xs = xs.tail;
        }
