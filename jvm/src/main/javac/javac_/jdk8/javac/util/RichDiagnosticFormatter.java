...
public class RichDiagnosticFormatter extends
        ForwardingDiagnosticFormatter<com.sun.tools.javac.util.JCDiagnostic, AbstractDiagnosticFormatter> {
    ...
    Map<WhereClauseKind, Map<Type, com.sun.tools.javac.util.JCDiagnostic>> whereClauses;

    /** Get the DiagnosticFormatter instance for this context. */
    public static RichDiagnosticFormatter instance(com.sun.tools.javac.util.Context context) {
    ...
    protected RichDiagnosticFormatter(Context context) {
        ...
        this.diags = com.sun.tools.javac.util.JCDiagnostic.Factory.instance(context);
        this.types = Types.instance(context);
        this.messages = JavacMessages.instance(context);
        whereClauses = new EnumMap<WhereClauseKind, Map<Type, com.sun.tools.javac.util.JCDiagnostic>>(WhereClauseKind.class);
        configuration = new RichConfiguration(com.sun.tools.javac.util.Options.instance(context), formatter);
        for (WhereClauseKind kind : WhereClauseKind.values())
            whereClauses.put(kind, new LinkedHashMap<Type, com.sun.tools.javac.util.JCDiagnostic>());
    }

    @Override
    public String format(com.sun.tools.javac.util.JCDiagnostic diag, Locale l) {
        ...
        if (getConfiguration().isEnabled(RichFormatterFeature.WHERE_CLAUSES)) {
            com.sun.tools.javac.util.List<com.sun.tools.javac.util.JCDiagnostic> clauses = getWhereClauses();
            String indent = formatter.isRaw() ? "" :
                formatter.indentString(DetailsInc);
            for (com.sun.tools.javac.util.JCDiagnostic d : clauses) {
    ...
    @Override
    public String formatMessage(com.sun.tools.javac.util.JCDiagnostic diag, Locale l) {
        nameSimplifier = new ClassNameSimplifier();
        preprocessDiagnostic(diag);
     ...
     */
    protected void preprocessDiagnostic(com.sun.tools.javac.util.JCDiagnostic diag) {
        ...
        if (diag.isMultiline()) {
            for (com.sun.tools.javac.util.JCDiagnostic d : diag.getSubdiagnostics())
    ...
    protected void preprocessArgument(Object arg) {
        ...
        else if (arg instanceof com.sun.tools.javac.util.JCDiagnostic) {
            preprocessDiagnostic((com.sun.tools.javac.util.JCDiagnostic)arg);
        }
     ...
     */
    protected com.sun.tools.javac.util.List<com.sun.tools.javac.util.JCDiagnostic> getWhereClauses() {
        com.sun.tools.javac.util.List<com.sun.tools.javac.util.JCDiagnostic> clauses = com.sun.tools.javac.util.List.nil();
        for (WhereClauseKind kind : WhereClauseKind.values()) {
            com.sun.tools.javac.util.List<com.sun.tools.javac.util.JCDiagnostic> lines = com.sun.tools.javac.util.List.nil();
            for (Map.Entry<Type, com.sun.tools.javac.util.JCDiagnostic> entry : whereClauses.get(kind).entrySet()) {
            ...
            if (!lines.isEmpty()) {
                ...
                if (lines.size() > 1)
                    key += ".1";
                com.sun.tools.javac.util.JCDiagnostic d = diags.fragment(key, whereClauses.get(kind).keySet());
                d = new com.sun.tools.javac.util.JCDiagnostic.MultilineDiagnostic(d, lines.reverse());
    ...
    protected class ClassNameSimplifier {
        ...
        Map<com.sun.tools.javac.util.Name, com.sun.tools.javac.util.List<Symbol>> nameClashes = new HashMap<com.sun.tools.javac.util.Name, com.sun.tools.javac.util.List<Symbol>>();

        /**
         * Add a name usage to the simplifier's internal cache
         */
        protected void addUsage(Symbol sym) {
            com.sun.tools.javac.util.Name n = sym.getSimpleName();
            com.sun.tools.javac.util.List<Symbol> conflicts = nameClashes.get(n);
            if (conflicts == null) {
                conflicts = com.sun.tools.javac.util.List.nil();
            }
        ...
        public String simplify(Symbol s) {
            ...
            if (!s.type.isCompound() && !s.type.isPrimitive()) {
                com.sun.tools.javac.util.List<Symbol> conflicts = nameClashes.get(s.getSimpleName());
                if (conflicts == null ||
                    (conflicts.size() == 1 &&
                    conflicts.contains(s))) {
                    com.sun.tools.javac.util.List<com.sun.tools.javac.util.Name> l = com.sun.tools.javac.util.List.nil();
                    Symbol s2 = s;
    ...
    protected Types.UnaryVisitor<Void> typePreprocessor =
        ...
        public Void visit(com.sun.tools.javac.util.List<Type> ts) {
            for (Type t : ts)
                visit(t);
            return null;
        }
        ...
        public Void visitCapturedType(CapturedType t, Void ignored) {
            if (indexOf(t, WhereClauseKind.CAPTURED) == -1) {
                ...
                com.sun.tools.javac.util.JCDiagnostic d = diags.fragment("where.captured"+ suffix, t, t.bound, t.lower, t.wildcard);
        ...
        public Void visitClassType(ClassType t, Void ignored) {
            if (t.isCompound()) {
                if (indexOf(t, WhereClauseKind.INTERSECTION) == -1) {
                    ...
                    com.sun.tools.javac.util.List<Type> interfaces = types.interfaces(t);
                    com.sun.tools.javac.util.JCDiagnostic d = diags.fragment("where.intersection", t, interfaces.prepend(supertype));
        ...
        public Void visitTypeVar(TypeVar t, Void ignored) {
            if (indexOf(t, WhereClauseKind.TYPEVAR) == -1) {
                ...
                com.sun.tools.javac.util.List<Type> bounds = (bound != null) &&
                ...
                if ((t.tsym.flags() & SYNTHETIC) == 0) {
                    ...
                    com.sun.tools.javac.util.JCDiagnostic d = diags.fragment("where.typevar" +
