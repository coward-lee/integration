...
public class TypeAnnotations {
     ...
     */
    public AnnotationType annotationType(com.sun.tools.javac.code.Attribute.Compound a, com.sun.tools.javac.code.Symbol s) {
        com.sun.tools.javac.code.Attribute.Compound atTarget =
            a.type.tsym.attribute(syms.annotationTargetType.tsym);
        if (atTarget == null) {
            return inferTargetMetaInfo(a, s);
        }
        com.sun.tools.javac.code.Attribute atValue = atTarget.member(names.value);
        if (!(atValue instanceof com.sun.tools.javac.code.Attribute.Array)) {
            Assert.error("annotationType(): bad @Target argument " + atValue +
                    " (" + atValue.getClass() + ")");
            return AnnotationType.DECLARATION; // error recovery
        }
        com.sun.tools.javac.code.Attribute.Array arr = (com.sun.tools.javac.code.Attribute.Array) atValue;
        boolean isDecl = false, isType = false;
        for (com.sun.tools.javac.code.Attribute app : arr.values) {
            if (!(app instanceof com.sun.tools.javac.code.Attribute.Enum)) {
                Assert.error("annotationType(): unrecognized Attribute kind " + app +
                        " (" + app.getClass() + ")");
                isDecl = true;
                continue;
            }
            com.sun.tools.javac.code.Attribute.Enum e = (com.sun.tools.javac.code.Attribute.Enum) app;
            if (e.value.name == names.TYPE) {
                if (s.kind == com.sun.tools.javac.code.Kinds.TYP)
                    isDecl = true;
            } else if (e.value.name == names.FIELD) {
                if (s.kind == com.sun.tools.javac.code.Kinds.VAR &&
                        s.owner.kind != com.sun.tools.javac.code.Kinds.MTH)
                    isDecl = true;
            } else if (e.value.name == names.METHOD) {
                if (s.kind == com.sun.tools.javac.code.Kinds.MTH &&
                        !s.isConstructor())
                    isDecl = true;
            } else if (e.value.name == names.PARAMETER) {
                if (s.kind == com.sun.tools.javac.code.Kinds.VAR &&
                        s.owner.kind == com.sun.tools.javac.code.Kinds.MTH &&
                        (s.flags() & com.sun.tools.javac.code.Flags.PARAMETER) != 0)
                    isDecl = true;
                ...
                if (s.kind == com.sun.tools.javac.code.Kinds.PCK)
                    isDecl = true;
            } else if (e.value.name == names.TYPE_USE) {
                if (s.kind == com.sun.tools.javac.code.Kinds.TYP ||
                        s.kind == com.sun.tools.javac.code.Kinds.VAR ||
                        (s.kind == com.sun.tools.javac.code.Kinds.MTH && !s.isConstructor() &&
                        !s.type.getReturnType().hasTag(com.sun.tools.javac.code.TypeTag.VOID)) ||
     ...
     */
    private static AnnotationType inferTargetMetaInfo(com.sun.tools.javac.code.Attribute.Compound a, com.sun.tools.javac.code.Symbol s) {
        return AnnotationType.DECLARATION;
    }
    ...
    private class TypeAnnotationPositions extends TreeScanner {
         ...
         */
        private void separateAnnotationsKinds(JCTree typetree, com.sun.tools.javac.code.Type type, com.sun.tools.javac.code.Symbol sym,
                                              TypeAnnotationPosition pos) {
            List<com.sun.tools.javac.code.Attribute.Compound> annotations = sym.getRawAttributes();
            ListBuffer<com.sun.tools.javac.code.Attribute.Compound> declAnnos = new ListBuffer<com.sun.tools.javac.code.Attribute.Compound>();
            ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound> typeAnnos = new ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound>();
            ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound> onlyTypeAnnos = new ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound>();

            for (com.sun.tools.javac.code.Attribute.Compound a : annotations) {
                ...
                case BOTH: {
                    declAnnos.append(a);
                    com.sun.tools.javac.code.Attribute.TypeCompound ta = toTypeCompound(a, pos);
                    typeAnnos.append(ta);
                    break;
                }
                case TYPE: {
            ...
            List<com.sun.tools.javac.code.Attribute.TypeCompound> typeAnnotations = typeAnnos.toList();
            ...
            if (sym.getKind() == ElementKind.METHOD) {
                ...
                if (sym.getQualifiedName().equals(names._this)) {
                    ...
                    List<com.sun.tools.javac.code.Type> oldArgs = methType.argtypes;
                    ListBuffer<com.sun.tools.javac.code.Type> newArgs = new ListBuffer<com.sun.tools.javac.code.Type>();
        ...
        private com.sun.tools.javac.code.Type typeWithAnnotations(final JCTree typetree, final com.sun.tools.javac.code.Type type,
                                                                  final List<com.sun.tools.javac.code.Attribute.TypeCompound> annotations,
                                                                  final List<com.sun.tools.javac.code.Attribute.TypeCompound> onlyTypeAnnotations) {
            ...
            if (type.hasTag(com.sun.tools.javac.code.TypeTag.ARRAY)) {
                com.sun.tools.javac.code.Type.ArrayType arType = (com.sun.tools.javac.code.Type.ArrayType) type.unannotatedType();
                com.sun.tools.javac.code.Type.ArrayType tomodify = new com.sun.tools.javac.code.Type.ArrayType(null, arType.tsym);
                ...
                while (arType.elemtype.hasTag(com.sun.tools.javac.code.TypeTag.ARRAY)) {
                    if (arType.elemtype.isAnnotated()) {
                        com.sun.tools.javac.code.Type aelemtype = arType.elemtype;
                        arType = (com.sun.tools.javac.code.Type.ArrayType) aelemtype.unannotatedType();
                        ArrayType prevToMod = tomodify;
                        tomodify = new com.sun.tools.javac.code.Type.ArrayType(null, arType.tsym);
                        prevToMod.elemtype = tomodify.annotatedType(arType.elemtype.getAnnotationMirrors());
                    } else {
                        arType = (com.sun.tools.javac.code.Type.ArrayType) arType.elemtype;
                        tomodify.elemtype = new com.sun.tools.javac.code.Type.ArrayType(null, arType.tsym);
                        tomodify = (com.sun.tools.javac.code.Type.ArrayType) tomodify.elemtype;
                    }
                    arTree = arrayTypeTree(arTree.elemtype);
                    depth = depth.append(TypePathEntry.ARRAY);
                }
                com.sun.tools.javac.code.Type arelemType = typeWithAnnotations(arTree.elemtype, arType.elemtype, annotations, onlyTypeAnnotations);
                tomodify.elemtype = arelemType;
                {
                    // All annotations share the same position; modify the first one.
                    com.sun.tools.javac.code.Attribute.TypeCompound a = annotations.get(0);
                    TypeAnnotationPosition p = a.position;
                    p.location = p.location.prependList(depth.toList());
                }
                typetree.type = toreturn;
                return toreturn;
            } else if (type.hasTag(com.sun.tools.javac.code.TypeTag.TYPEVAR)) {
                ...
                com.sun.tools.javac.code.Type res = typeWithAnnotations(fst, fst.type, annotations, onlyTypeAnnotations);
                fst.type = res;
                ...
                com.sun.tools.javac.code.Type enclTy = type;
                ...
                com.sun.tools.javac.code.Type topTy = enclTy;
                while (enclEl != null &&
                        enclEl.getKind() != ElementKind.PACKAGE &&
                        topTy != null &&
                ...
                com.sun.tools.javac.code.Type ret = typeWithAnnotations(type, enclTy, annotations);
                typetree.type = ret;
                return ret;
            }
        }
         ...
         */
        private com.sun.tools.javac.code.Type typeWithAnnotations(final com.sun.tools.javac.code.Type type,
                                                                  final com.sun.tools.javac.code.Type stopAt,
                                                                  final List<com.sun.tools.javac.code.Attribute.TypeCompound> annotations) {
            Visitor<com.sun.tools.javac.code.Type, List<TypeCompound>> visitor =
                    new com.sun.tools.javac.code.Type.Visitor<com.sun.tools.javac.code.Type, List<com.sun.tools.javac.code.Attribute.TypeCompound>>() {
                @Override
                public com.sun.tools.javac.code.Type visitClassType(ClassType t, List<TypeCompound> s) {
                    // assert that t.constValue() == null?
                    if (t == stopAt ||
                        t.getEnclosingType() == com.sun.tools.javac.code.Type.noType) {
                ...
                @Override
                public com.sun.tools.javac.code.Type visitAnnotatedType(AnnotatedType t, List<TypeCompound> s) {
                    return t.unannotatedType().accept(this, s).annotatedType(t.getAnnotationMirrors());
                }

                @Override
                public com.sun.tools.javac.code.Type visitWildcardType(WildcardType t, List<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public com.sun.tools.javac.code.Type visitArrayType(ArrayType t, List<TypeCompound> s) {
                    ArrayType ret = new ArrayType(t.elemtype.accept(this, s), t.tsym);
                    return ret;
                }

                @Override
                public com.sun.tools.javac.code.Type visitMethodType(MethodType t, List<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public com.sun.tools.javac.code.Type visitPackageType(PackageType t, List<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public com.sun.tools.javac.code.Type visitTypeVar(TypeVar t, List<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public com.sun.tools.javac.code.Type visitCapturedType(CapturedType t, List<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public com.sun.tools.javac.code.Type visitForAll(ForAll t, List<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public com.sun.tools.javac.code.Type visitUndetVar(UndetVar t, List<TypeCompound> s) {
                    // Impossible?
                    return t;
                }

                @Override
                public com.sun.tools.javac.code.Type visitErrorType(ErrorType t, List<TypeCompound> s) {
                    return t.annotatedType(s);
                }

                @Override
                public com.sun.tools.javac.code.Type visitType(com.sun.tools.javac.code.Type t, List<TypeCompound> s) {
                    return t.annotatedType(s);
                }
            };
        ...
        private com.sun.tools.javac.code.Attribute.TypeCompound toTypeCompound(com.sun.tools.javac.code.Attribute.Compound a, TypeAnnotationPosition p) {
            // It is safe to alias the position.
            return new com.sun.tools.javac.code.Attribute.TypeCompound(a, p);
        }
            ...
            */
            ...
            switch (frame.getKind()) {
                case TYPE_CAST:
                    ...
                    p.type = com.sun.tools.javac.code.TargetType.CAST;
                ...
                case INSTANCE_OF:
                    p.type = com.sun.tools.javac.code.TargetType.INSTANCEOF;
                    p.pos = frame.pos;
                    return;
                ...
                case NEW_CLASS:
                    ...
                    if (frameNewClass.def != null) {
                        ...
                        if (frameClassDecl.extending == tree) {
                            p.type = com.sun.tools.javac.code.TargetType.CLASS_EXTENDS;
                            p.type_index = -1;
                        ...
                        p.type = com.sun.tools.javac.code.TargetType.CONSTRUCTOR_INVOCATION_TYPE_ARGUMENT;
                        p.type_index = frameNewClass.typeargs.indexOf(tree);
                    } else {
                        p.type = com.sun.tools.javac.code.TargetType.NEW;
                    }
                    p.pos = frame.pos;
                    return;
                ...
                case INTERFACE:
                    ...
                    if (((JCClassDecl)frame).extending == tree) {
                        ...
                        p.type = com.sun.tools.javac.code.TargetType.CLASS_TYPE_PARAMETER;
                ...
                case METHOD: {
                    ...
                    if (frameMethod.thrown.contains(tree)) {
                        p.type = com.sun.tools.javac.code.TargetType.THROWS;
                        p.type_index = frameMethod.thrown.indexOf(tree);
                    } else if (frameMethod.restype == tree) {
                        p.type = com.sun.tools.javac.code.TargetType.METHOD_RETURN;
                    } else if (frameMethod.typarams.contains(tree)) {
                        p.type = com.sun.tools.javac.code.TargetType.METHOD_TYPE_PARAMETER;
                ...
                case MEMBER_REFERENCE: {
                    ...
                    if (mrframe.expr == tree) {
                        switch (mrframe.mode) {
                        case INVOKE:
                            p.type = com.sun.tools.javac.code.TargetType.METHOD_REFERENCE;
                            break;
                        case NEW:
                            p.type = com.sun.tools.javac.code.TargetType.CONSTRUCTOR_REFERENCE;
                            break;
                        default:
                        ...
                        switch (mrframe.mode) {
                        case INVOKE:
                            p.type = com.sun.tools.javac.code.TargetType.METHOD_REFERENCE_TYPE_ARGUMENT;
                            break;
                        case NEW:
                            p.type = com.sun.tools.javac.code.TargetType.CONSTRUCTOR_REFERENCE_TYPE_ARGUMENT;
                            break;
                        default:
                            Assert.error("Unknown method reference mode " + mrframe.mode +
                                    " for tree " + tree + " within frame " + frame);
                ...
                case TYPE_PARAMETER:
                    if (path.tail.tail.head.hasTag(JCTree.Tag.CLASSDEF)) {
                        ...
                        p.type = com.sun.tools.javac.code.TargetType.CLASS_TYPE_PARAMETER_BOUND;
                        ...
                        p.type = com.sun.tools.javac.code.TargetType.METHOD_TYPE_PARAMETER_BOUND;
                ...
                case VARIABLE:
                    ...
                    switch (v.getKind()) {
                        case LOCAL_VARIABLE:
                            p.type = com.sun.tools.javac.code.TargetType.LOCAL_VARIABLE;
                            break;
                        case FIELD:
                            p.type = com.sun.tools.javac.code.TargetType.FIELD;
                            break;
                        case PARAMETER:
                            if (v.getQualifiedName().equals(names._this)) {
                                // TODO: Intro a separate ElementKind?
                                p.type = com.sun.tools.javac.code.TargetType.METHOD_RECEIVER;
                            } else {
                                p.type = com.sun.tools.javac.code.TargetType.METHOD_FORMAL_PARAMETER;
                                p.parameter_index = methodParamIndex(path, frame);
                            }
                            break;
                        case EXCEPTION_PARAMETER:
                            p.type = com.sun.tools.javac.code.TargetType.EXCEPTION_PARAMETER;
                            break;
                        case RESOURCE_VARIABLE:
                            p.type = com.sun.tools.javac.code.TargetType.RESOURCE_VARIABLE;
                            break;
                        default:
                ...
                case ANNOTATED_TYPE: {
                    if (frame == tree) {
                        ...
                        final com.sun.tools.javac.code.Type utype = atypetree.underlyingType.type;
                        if (utype == null) {
                            // This might happen during DeferredAttr;
                            // we will be back later.
                            return;
                        }
                        com.sun.tools.javac.code.Symbol tsym = utype.tsym;
                ...
                case METHOD_INVOCATION: {
                    ...
                    if (exsym == null) {
                        ...
                        p.type = com.sun.tools.javac.code.TargetType.METHOD_INVOCATION_TYPE_ARGUMENT;
                    }
        ...
        private void locateNestedTypes(com.sun.tools.javac.code.Type type, TypeAnnotationPosition p) {
            // The number of "steps" to get from the full type to the
            // left-most outer type.
            ListBuffer<TypePathEntry> depth = new ListBuffer<>();

            com.sun.tools.javac.code.Type encl = type.getEnclosingType();
            while (encl != null &&
        ...
        public void visitMethodDef(final JCMethodDecl tree) {
            ...
            if (sigOnly) {
                if (!tree.mods.annotations.isEmpty()) {
                    ...
                    pos.type = com.sun.tools.javac.code.TargetType.METHOD_RETURN;
                ...
                if (tree.recvparam != null && tree.recvparam.sym != null &&
                    ...
                    TypeAnnotationPosition pos = new TypeAnnotationPosition();
                    pos.type = com.sun.tools.javac.code.TargetType.METHOD_RECEIVER;
                ...
                for (JCVariableDecl param : tree.params) {
                    if (!param.mods.annotations.isEmpty()) {
                        ...
                        // there are no annotations of either kind.
                        TypeAnnotationPosition pos = new TypeAnnotationPosition();
                        pos.type = com.sun.tools.javac.code.TargetType.METHOD_FORMAL_PARAMETER;
        ...
        public void visitVarDef(final JCVariableDecl tree) {
            if (tree.mods.annotations.isEmpty()) {
                ...
                if (sigOnly) {
                    TypeAnnotationPosition pos = new TypeAnnotationPosition();
                    pos.type = com.sun.tools.javac.code.TargetType.FIELD;
                    pos.pos = tree.pos;
                    separateAnnotationsKinds(tree.vartype, tree.sym.type, tree.sym, pos);
                }
            } else if (tree.sym.getKind() == ElementKind.LOCAL_VARIABLE) {
                TypeAnnotationPosition pos = new TypeAnnotationPosition();
                pos.type = com.sun.tools.javac.code.TargetType.LOCAL_VARIABLE;
                pos.pos = tree.pos;
                pos.onLambda = currentLambda;
                separateAnnotationsKinds(tree.vartype, tree.sym.type, tree.sym, pos);
            } else if (tree.sym.getKind() == ElementKind.EXCEPTION_PARAMETER) {
                TypeAnnotationPosition pos = new TypeAnnotationPosition();
                pos.type = com.sun.tools.javac.code.TargetType.EXCEPTION_PARAMETER;
                pos.pos = tree.pos;
                pos.onLambda = currentLambda;
                separateAnnotationsKinds(tree.vartype, tree.sym.type, tree.sym, pos);
            } else if (tree.sym.getKind() == ElementKind.RESOURCE_VARIABLE) {
                TypeAnnotationPosition pos = new TypeAnnotationPosition();
                pos.type = com.sun.tools.javac.code.TargetType.RESOURCE_VARIABLE;
                pos.pos = tree.pos;
                pos.onLambda = currentLambda;
                separateAnnotationsKinds(tree.vartype, tree.sym.type, tree.sym, pos);
        ...
        private void copyNewClassAnnotationsToOwner(JCNewClass tree) {
            ...
            TypeAnnotationPosition pos = new TypeAnnotationPosition();
            ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound> newattrs =
                new ListBuffer<com.sun.tools.javac.code.Attribute.TypeCompound>();

            for (com.sun.tools.javac.code.Attribute.TypeCompound old : sym.getRawTypeAttributes()) {
                newattrs.append(new com.sun.tools.javac.code.Attribute.TypeCompound(old.type, old.values,
                                                           pos));
            }

            pos.type = com.sun.tools.javac.code.TargetType.NEW;
            pos.pos = tree.pos;
        ...
        public void visitNewClass(JCNewClass tree) {
            if (tree.def != null &&
                ...
                TypeAnnotationPosition pos = new TypeAnnotationPosition();
                pos.type = com.sun.tools.javac.code.TargetType.CLASS_EXTENDS;
                pos.pos = tree.pos;
