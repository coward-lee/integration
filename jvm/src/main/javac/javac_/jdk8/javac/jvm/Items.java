...
public class Items {
    ...
    public Items(Pool pool, com.sun.tools.javac.jvm.Code code, Symtab syms, Types types) {
        this.code = code;
        this.pool = pool;
        this.types = types;
    ...
    Item makeStackItem(Type type) {
        return stackItem[com.sun.tools.javac.jvm.Code.typecode(type)];
    }
    ...
    abstract class Item {
        ...
        Item coerce(int targetcode) {
            if (typecode == targetcode)
                ...
                load();
                int typecode1 = com.sun.tools.javac.jvm.Code.truncate(typecode);
                int targetcode1 = com.sun.tools.javac.jvm.Code.truncate(targetcode);
        ...
        Item coerce(Type targettype) {
            return coerce(com.sun.tools.javac.jvm.Code.typecode(targettype));
        }
    ...
    class StackItem extends Item {
        ...
        void stash(int toscode) {
            code.emitop0(
                (width() == 2 ? dup_x2 : dup_x1) + 3 * (com.sun.tools.javac.jvm.Code.width(toscode) - 1));
        }

        int width() {
            return com.sun.tools.javac.jvm.Code.width(typecode);
        }
    ...
    class IndexedItem extends Item {
        ...
        IndexedItem(Type type) {
            super(com.sun.tools.javac.jvm.Code.typecode(type));
        }
        ...
        void stash(int toscode) {
            code.emitop0(dup_x2 + 3 * (com.sun.tools.javac.jvm.Code.width(toscode) - 1));
        }
    ...
    class LocalItem extends Item {
        ...
        Item load() {
            if (reg <= 3)
                code.emitop0(iload_0 + com.sun.tools.javac.jvm.Code.truncate(typecode) * 4 + reg);
            else
                code.emitop1w(iload + com.sun.tools.javac.jvm.Code.truncate(typecode), reg);
            return stackItem[typecode];
        }

        void store() {
            if (reg <= 3)
                code.emitop0(istore_0 + com.sun.tools.javac.jvm.Code.truncate(typecode) * 4 + reg);
            else
                code.emitop1w(istore + com.sun.tools.javac.jvm.Code.truncate(typecode), reg);
            code.setDefined(reg);
        }
    ...
    class StaticItem extends Item {
        ...
        StaticItem(Symbol member) {
            super(com.sun.tools.javac.jvm.Code.typecode(member.erasure(types)));
            this.member = member;
        }
        ...
        Item invoke() {
            ...
            int rescode = com.sun.tools.javac.jvm.Code.typecode(mtype.restype);
    ...
    class ImmediateItem extends Item {
        ...
        Item coerce(int targetcode) {
            if (typecode == targetcode) {
                ...
                switch (targetcode) {
                case INTcode:
                    if (com.sun.tools.javac.jvm.Code.truncate(typecode) == INTcode)
                        return this;
                    else
    ...
    class AssignItem extends Item {
        ...
        int width() {
            return lhs.width() + com.sun.tools.javac.jvm.Code.width(typecode);
        }
    ...
    class CondItem extends Item {
        ...
        Chain jumpTrue() {
            if (tree == null) return com.sun.tools.javac.jvm.Code.mergeChains(trueJumps, code.branch(opcode));
            // we should proceed further in -Xjcov mode only
            int startpc = code.curCP();
            Chain c = com.sun.tools.javac.jvm.Code.mergeChains(trueJumps, code.branch(opcode));
            code.crt.put(tree, CRTable.CRT_BRANCH_TRUE, startpc, code.curCP());
            return c;
        }

        Chain jumpFalse() {
            if (tree == null) return com.sun.tools.javac.jvm.Code.mergeChains(falseJumps, code.branch(com.sun.tools.javac.jvm.Code.negate(opcode)));
            // we should proceed further in -Xjcov mode only
            int startpc = code.curCP();
            Chain c = com.sun.tools.javac.jvm.Code.mergeChains(falseJumps, code.branch(com.sun.tools.javac.jvm.Code.negate(opcode)));
            code.crt.put(tree, CRTable.CRT_BRANCH_FALSE, startpc, code.curCP());
            return c;
        }

        CondItem negate() {
            CondItem c = new CondItem(com.sun.tools.javac.jvm.Code.negate(opcode), falseJumps, trueJumps);
            c.tree = tree;
            return c;
        }
