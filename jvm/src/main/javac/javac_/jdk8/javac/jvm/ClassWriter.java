 ...
 ******************************************************************/
     ...
     */
    void writePool(com.sun.tools.javac.jvm.Pool pool) throws PoolOverflow, StringOverflow {
        ...
        while (i < pool.pp) {
            ...
            if (value instanceof MethodSymbol) {
                ...
                if (bs.length > com.sun.tools.javac.jvm.Pool.MAX_STRING_LENGTH)
    ...
    ClassFile.NameAndType nameType(Symbol sym) {
        return new ClassFile.NameAndType(fieldName(sym),
                               retrofit
                               ? sym.erasure(types)
...
//          log.errWriter.println("enter inner " + c);//DEBUG
     ...
     */
    void writeCode(com.sun.tools.javac.jvm.Code code) {
        ...
        if (code.varDebugInfo && code.varBufferSize > 0) {
            ...
            for (int i=0; i<code.varBufferSize; i++) {
                com.sun.tools.javac.jvm.Code.LocalVar var = code.varBuffer[i];

                for (com.sun.tools.javac.jvm.Code.LocalVar.Range r: var.aliveRanges) {
                    // write variable info
    ...
    void writeStackMap(com.sun.tools.javac.jvm.Code code) {
        ...
        case CLDC:
            for (int i=0; i<nframes; i++) {
                ...
                com.sun.tools.javac.jvm.Code.StackMapFrame frame = code.stackMapBuffer[i];
                ...
                for (int j=0; j<frame.locals.length;
                     j += (target.generateEmptyAfterBig() ? 1 : com.sun.tools.javac.jvm.Code.width(frame.locals[j]))) {
                    localCount++;
                }
                ...
                for (int j=0; j<frame.stack.length;
                     j += (target.generateEmptyAfterBig() ? 1 : com.sun.tools.javac.jvm.Code.width(frame.stack[j]))) {
                    stackCount++;
                }
